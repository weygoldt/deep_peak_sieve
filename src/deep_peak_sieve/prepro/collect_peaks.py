...
from deep_peak_sieve.prepro.filters import bandpass_filter
...
def load_data(path: Path, filetype="wav"):
    """
    ...
    if not path.exists():
        ...
        files = sorted(list(path.glob(f"*.{filetype}")))
        files = [str(file) for file in files]
        data = AudioLoader(files)
    elif path.is_dir() and len(list(path.glob(f"*.{filetype}"))) == 0:
        msg = (
            f"Path {path} is a directory but no files with extension "
            f"{filetype} were found."
        )
...
def collect_peaks(
    ...
    6) Dataset saving
    """
    data = load_data(path=path, filetype=filetype)
