...
def select_channels(channels: npt.NDArray, probe_selected_channels: Optional[dict], probe_frame:nixio.DataFrame):
    ...
    channels = probe_frame["contact_ids"][sorted_after_y_pos][channels].astype(np.int32) - 1

    if probe_selected_channels:
        if probe_selected_channels["points"]:
            channels = np.zeros(len(probe_selected_channels["points"]), dtype=np.int32)
