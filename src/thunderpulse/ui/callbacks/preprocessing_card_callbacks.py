...
def callbacks(app):
    @app.callback(
        Output("minimum_distance_channel", "children"), Input("filepath", "data")
    )
    def minimum_distance_channel(filepath):
        ...
        if not filepath["data_path"]:
            return None
        nix_file = nixio.File.open(filepath["data_path"], nixio.FileMode.ReadOnly)
        ...
        return f"The minimal distance from channel to channel is {minimum[0]} um"

    @app.callback(Output("p_threshold", "children"), Input("n_median", "value"))
